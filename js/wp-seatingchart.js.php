...
function RoomItem(g_itemTypeID, g_claimable, g_claimedBy, g_x, g_y, g_direction, g_zIndex, g_seatingChartID)
{
	...
	this.Draw = function(canvas, zoom, admin)
	{
		...
		else
		{
			...
			this.Image.toJSON = function(){return"";};
			...
			this.Container.toJSON = function(){return"";};
					...
					this.Container.hover(
						function()
						{
							...
							else
							{
								...
								if (room.RoomItems[$j(this).attr("id")].ClaimedBy == 0 && !room.IsSitting(currentUserId))
								{
...
function Room(g_width, g_height, g_zoom, canvas, admin)
{
	...
	this.RoomItems = {};
	this.Canvas = canvas;
	this.Canvas.toJSON = function(){return"";};
	this.Admin = admin;
	...
	this.IsSitting = function(userID)
	{
		...
		{
			if (this.RoomItems[roomItemIndex].ClaimedBy == userID) return true;
		}		
		return false;
	};
