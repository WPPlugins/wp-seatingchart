...
Plugin URI: 
Description: Create a seating layout using tables, chairs, restroom locations and trash recepticles via a drag-n-drop interface.  Allow users to reserve a seat.
Version: 1.0.4
Author: oneprince
Author URI:
License: GPL2
...
/*  Copyright 2010  oneprince  (email : oneprince@gmail.com)
    ...
    class WPSeatingChartPluginSeries {
	    ...
	    function installDatabase()
		{
		    ...
		    $wpdb->query("INSERT INTO $table(wpsc_ItemName, wpsc_ImageN, wpsc_ImageW, wpsc_ImageE, wpsc_ImageS, wpsc_Claimable)
		        VALUES('Table', 'table_vertical.png', 'table_horizontal.png', 'table_horizontal.png', 'table_vertical.png', 0 )");
        ...
        function userSitDown()
        {
			...
			//
			//	make sure the user isn't already sitting in another seat
			//
			...
			if (!empty($claimedByYouAlready) && $claimedByYouAlready[0]->cnt > 0)
			{
				echo "You are already sitting, please refresh the page.";
			}
			else if (!empty($claimedAlready) && $claimedAlready[0]->cnt > 0)
			{
				echo "Someone snagged the seat before you, please refresh the page.";
			}
			else
			{			
