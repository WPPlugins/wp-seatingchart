...
foreach ($seatingchartitems as $seatingchartitem)
{
	...
	list($width, $height, $type, $attr) = getimagesize(WP_PLUGIN_URL . "/wp-seatingchart/images/" . $seatingchartitem->wpsc_ImageN);
	...
	list($width, $height, $type, $attr) = getimagesize(WP_PLUGIN_URL . "/wp-seatingchart/images/" . $seatingchartitem->wpsc_ImageE);
	$seatingchartitem->wpsc_ImageEWidth = $width;
	$seatingchartitem->wpsc_ImageEHeight = $height;

	list($width, $height, $type, $attr) = getimagesize(WP_PLUGIN_URL . "/wp-seatingchart/images/" . $seatingchartitem->wpsc_ImageS);
	$seatingchartitem->wpsc_ImageSWidth = $width;
	$seatingchartitem->wpsc_ImageSHeight = $height;

	list($width, $height, $type, $attr) = getimagesize(WP_PLUGIN_URL . "/wp-seatingchart/images/" . $seatingchartitem->wpsc_ImageW);
...
{ 
	...
	if ($json_array["RoomItems"] != null)
	{
		foreach ($json_array["RoomItems"] as $key => $value)
		{
			...
			$sql = $wpdb->prepare("INSERT INTO $wpsc_seatingchart_table (wpsc_ItemTypeID, wpsc_Claimable, wpsc_ClaimedBy, wpsc_X, wpsc_Y, wpsc_Direction, wpsc_ZIndex) VALUES (%d, %d, %d, %d, %d, %s, %d)",
								$value["ItemTypeID"], $value["Claimable"], $value["ClaimedBy"], $value["Left"], $value["Top"], $value["Direction"], $value["ZIndex"]);
...
<div class="wrap">
	<form id="wpsc_form" method="post" action="<?php echo $_SERVER["REQUEST_URI"]; ?>">
		...
		<div class="wpsc_inputItem">
			<h3>Room Width</h3>
			<input readonly type="text" id="wpsc_room_size_width" name="wpsc_room_size_width" value="<?php _e(apply_filters('format_to_edit',$wpscOptions['wpsc_room_size_width']), 'wp-seatingchart') ?>"></input>
			<div id="sliderWidth"> 
			</div>
		</div>
		...
		<div class="wpsc_inputItem">
			<h3>Room Height</h3>
			<input readonly type="text" id="wpsc_room_size_height" name="wpsc_room_size_height" value="<?php _e(apply_filters('format_to_edit',$wpscOptions['wpsc_room_size_height']), 'wp-seatingchart') ?>"></input>
			<div id="sliderHeight"> 
			</div>
		</div>
		...
		<div class="wpsc_inputItem">		
			<h3>Room Zoom</h3>
			<input readonly type="text" id="wpsc_room_size_item_zoom" name="wpsc_room_size_item_zoom" value="<?php _e(apply_filters('format_to_edit',$wpscOptions['wpsc_room_size_item_zoom']), 'wp-seatingchart') ?>"></input>
			<div id="sliderZoom"> 
			</div>
		</div>
		...
		<div class="wpsc_inputItem" id="divItemDetails">
			<h3>Item Details</h3>
			Claimable: 
			<input type="checkbox" id="wpsc_item_claimable" name="wpsc_item_claimable"></input>
			<br/>
			<div id="divClaimedBy">
				Claimed By:
